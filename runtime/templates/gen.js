...
let $buildBase64String = function($rawBytes) {
	...
	let $chars = PST$createNewArray(64, null);
	let $j = 0;
	while ($j < 26) {
...
let $buildIntegerListValue = function($ec, $ints) {
	let $sz = $ints.length;
	let $g = $ec[1];
	let $vals = PST$createNewArray($sz, null);
	let $n = 0;
	let $val = null;
	let $i = 0;
	while ($i < $sz) {
...
let $buildList = function($ec, $values, $copyValues, $lengthOrNegativeOne) {
	...
	if ($copyValues) {
		let $buf = PST$createNewArray($size, null);
		let $i = 0;
		while ($i < $size) {
			$buf[$i] = $values[$i];
			$i += 1;
...
let $convertListToByteArray = function($list) {
	let $sz = $list[1];
	let $bytesOut = PST$createNewArray($sz, 0);
	let $val = 0;
	let $value = null;
	let $i = 0;
	while ($i < $sz) {
...
let $createClassInfo = function($id, $parentId, $name, $ctor, $newFields, $newMemberInfoFlags, $cctorIdOrZero) {
	...
	let $info = [$id, $parentId, null, $name, $ctorVal, $cctorIdOrZero, false, {}, {}, PST$createNewArray($fieldCount, null), PST$createNewArray($fieldCount, null), $newFields, $newMemberInfoFlags, null, {}, {}];
	$info[13] = [13, $info];
	return $info;
};
...
let $createMainTask = function($ec, $cliArgs) {
	...
	let $args = $ec[1][0];
	let $argList = PST$createNewArray(1, null);
	let $argValues = PST$createNewArray($cliArgs.length, null);
	let $i = 0;
...
let $createNewTask = function($ec, $fpValue, $args) {
	...
	let $argsClone = PST$createNewArray($argc, null);
	let $i = 0;
	while ($i < $argc) {
		$argsClone[$i] = $args[$i];
		$i += 1;
	...
	let $task = [$ec[18], $ec, $frame, false, 0, PST$createNewArray(1, null), null];
	$ec[18] += 1;
	$ec[20][$task[0]] = $task;
	return $task;
};
...
let $createStringFromUnicodeArray = function($g, $buffer, $copyBuffer) {
	...
	if ($copyBuffer) {
		$uchars = PST$createNewArray($sz, 0);
		let $i = 0;
		while ($i < $sz) {
			$uchars[$i] = $buffer[$i];
			$i += 1;
...
let $createStringFromUnicodeArraySegment = function($g, $buffer, $start, $length) {
	...
	let $newBuf = PST$createNewArray($length, 0);
	let $i = 0;
	while ($i < $length) {
...
let $DictImpl_clone = function($ec, $original) {
	...
	if ($sz > 0) {
		$output[3] = $sz;
		$output[4] = PST$createNewArray($sz, null);
		$output[5] = PST$createNewArray($sz, null);
...
let $DictImpl_ensureCapacity = function($dict) {
	...
	let $newKeys = PST$createNewArray($newCapacity, null);
	let $newValues = PST$createNewArray($newCapacity, null);
	let $oldKeys = $dict[4];
...
let $ExRes_HardCrash = function($task, $message) {
	...
	if ($isDebug == 1) {
		...
		$failArgs[0] = $message;
		let $ignore = PST$extCallbacks["hardCrash"]($failArgs);
	}
	return $res;
};
...
let $generateNameLookup = function($ec) {
	...
	let $deadArray = PST$createNewArray(16, null);
	let $fpMap = PST$createNewArray($sz, null);
	$lookup[1] = $fpMap;
...
let $generateStackTrace = function($task) {
	...
	let $sz = $trace.length;
	let $traceArr = PST$createNewArray($sz, null);
	let $i = 0;
	while ($i < $sz) {
...
let $generateTryDescriptors = function($ec) {
	let $bc = $ec[11];
	let $sz = $bc.length;
	$ec[12] = PST$createNewArray($sz, null);
	let $i = 0;
	while ($i < $sz) {
		$ec[12][$i] = null;
		let $row = $bc[$i];
...
let $injectNameLookup = function($lookup, $primitiveMethodId, $typeId, $nameId, $argcMin, $argcMax) {
	...
	if ($lookup[$nameId] == null) {
		$lookup[$nameId] = PST$createNewArray(16, null);
	}
...
let $json_util_parse = function($ec, $rawValue, $errOut) {
	...
	$errOut[1] = 0;
	$errOut[2] = 0;
	let $args = PST$createNewArray(3, null);
	$args[0] = $ec;
	$args[1] = $rawValue;
	$args[2] = $errOut;
	let $resultObj = PST$extCallbacks["jsonParse"]($args);
	if ($errOut[0] == 1) {
		return null;
	}
	return $resultObj;
...
let $List_expandCapacity = function($o) {
	...
	let $newItems = PST$createNewArray($newCapacity, null);
	let $oldItems = $o[3];
	let $i = 0;
	while ($i < $oldCapacity) {
...
let $new_ExecutionContext = function($rawBytes, $extensions, $appCtx) {
	...
	if ($err == null) {
		...
		let $i = 0;
		$ec[5] = PST$createNewArray($ec[4].length, null);
		$i = 1;
		while ($i < $ec[4].length) {
			$ec[5][$i] = $buildFunctionFromInfo($ec[4][$i]);
			$i += 1;
		}
		$ec[7] = PST$createNewArray($ec[6].length, null);
		$i = 1;
		while ($i < $ec[6].length) {
			$ec[7][$i] = $buildFunctionFromInfo($ec[6][$i]);
			$i += 1;
		}
		$ec[9] = PST$createNewArray($ec[8].length, null);
		$i = 1;
		while ($i < $ec[8].length) {
			$ec[9][$i] = [13, $ec[8][$i]];
			$i += 1;
		}
		$ec[15] = PST$createNewArray($ec[11].length, null);
		$ec[16] = PST$createNewArray($ec[11].length, null);
...
let $new_GlobalValues = function() {
	let $g = [[1, null], [2, true], [2, false], null, null, null, {}, null, null, PST$createNewArray(5, null)];
	$g[3] = PST$createNewArray(1200, null);
	$g[4] = PST$createNewArray(1200, null);
	let $i = 0;
	while ($i < 1200) {
		$g[3][$i] = [3, $i];
		$g[4][$i] = [3, -$i];
...
let $ParseRaw_entitiesSection_classMemberResolver = function($classes, $globalValues) {
	...
	while ($id < $classes.length) {
		...
		let $nameByOffset = PST$createNewArray($fieldCount, null);
		let $flatOffset = 0;
		...
		$cd[9] = $nameByOffset;
		$cd[8] = {};
		$cd[10] = PST$createNewArray($fieldCount, null);
		let $i = 0;
		while ($i < $fieldCount) {
...
let $ParseRaw_entitiesSection_parseEnums = function($rdp, $enumCount) {
	...
	while ($i < $enumCount) {
		...
		let $id = $i + 1;
		let $enumValues = PST$createNewArray($memberCount, 0);
		let $enumNames = PST$createNewArray($memberCount, null);
		let $j = 0;
		while ($j < $memberCount) {
...
let $ParseRaw_parseStringData = function($rdp) {
	...
	let $count = $rdp[3];
	let $output = PST$createNewArray($count + 1, null);
	$output[0] = null;
	let $i = 0;
	while ($i < $count) {
...
let $ParseRaw_popByteCodeRows = function($rdp, $rowCount) {
	let $rows = PST$createNewArray($rowCount, null);
	let $i = 0;
	while ($i < $rowCount) {
		if (!$ParseRaw_popInt($rdp)) {
			return null;
		...
		let $args = PST$createNewArray($argc, 0);
		let $j = 0;
		while ($j < $argc) {
			if (!$ParseRaw_popInt($rdp)) {
				return null;
...
let $ParseRaw_popBytes = function($rdp, $byteCount) {
	...
	let $output = PST$createNewArray($byteCount, 0);
	let $i = 0;
	while ($i < $byteCount) {
...
let $ParseRaw_popFixedLenString = function($rdp, $expectedSize) {
	...
	let $strBytes = PST$createNewArray($expectedSize, 0);
	let $i = 0;
...
let $PUBLIC_createTaskForFunction = function($ecCtx, $fpValueNoArgs) {
	return $PUBLIC_createTaskForFunctionWithWrappedArgs($ecCtx, $fpValueNoArgs, PST$createNewArray(0, null));
};

let $PUBLIC_createTaskForFunctionWithWrappedArgs = function($ecCtx, $fpValue, $wrappedArgs) {
	let $argc = $wrappedArgs.length;
	let $args = PST$createNewArray($argc, null);
	let $i = 0;
	while ($i < $argc) {
...
let $RunInterpreterImpl = function($task) {
	...
	let $value16 = PST$createNewArray(16, null);
	let $intBuffer16 = PST$createNewArray(16, 0);
	let $floatBuffer16 = PST$createNewArray(16, 0);
	while (true) {
		$row = $byteCode[$pc];
		switch ($row[0]) {
			case 1:
				...
				switch ($left[0] * 16 + $right[0]) {
					...
					case 165:
						...
						if ($dictImpl1[1] == 1) {
							...
							$dictImpl1[7] = {};
							$dictImpl1[7][$str1] = 0;
							$dictImpl1[4] = PST$createNewArray(4, null);
							$dictImpl1[5] = PST$createNewArray(4, null);
							$dictImpl1[3] = 4;
							$dictImpl1[4][0] = $right;
							$dictImpl1[5][0] = $value;
							$dictImpl1[2] = 1;
			...
			case 13:
				...
				if ($sz == 0) {
					...
					$int2 = $sz;
					$keys = PST$createNewArray($sz, null);
					$values = PST$createNewArray($sz, null);
					$valueStackSize -= $sz * 2;
					$j = $valueStackSize;
					$i = 0;
					while ($i < $sz) {
			...
			case 12:
				// OP_BUILD_LIST;
				$sz = $row[2];
				$args = PST$createNewArray($sz, null);
				$valueStackSize -= $sz;
				$i = 0;
				while ($i < $sz) {
			...
			case 20:
				...
				if ($ec[2][$name] !== undefined) {
					$objArr = PST$createNewArray($argc, null);
					$i = $argc - 1;
					while ($i >= 0) {
						$valueStackSize -= 1;
			...
			case 21:
				// OP_FUNCTION_INVOKE;
				$argc = $row[2];
				$args = PST$createNewArray($argc, null);
				$i = $argc - 1;
				while ($i >= 0) {
					$valueStackSize -= 1;
					...
					case 4:
						...
						switch ($fp[3]) {
							...
							case 24:
								$listImpl1 = $fp[5][1];
								$sz = $listImpl1[1];
								$intArray1 = PST$createNewArray($sz, 0);
								$i = 0;
								while ($i < $sz) {
									$left = $listImpl1[3][$i];
									if ($left[0] != 3) {
							...
							case 36:
								$stringImpl1 = $fp[5][1];
								$sz = $stringImpl1[0];
								$valueArr = PST$createNewArray($sz, null);
			...
			case 22:
				...
				switch ($left[0] * 16 + $right[0]) {
					case 83:
						...
						$intArray1 = PST$createNewArray(1, 0);
			...
			case 37:
				...
				if ($classDef[6]) {
					...
					if ($classDef[5] > 0) {
						...
						$frame = [$frame, $pc, PST$createNewArray(0, null), 0, $valueStackSize, $valueStackSize, {}, $classDef[13], true, null, false];
						$locals = $frame[6];
						$task[2] = $frame;
					} else {
						$pc -= 1;
			...
			case 48:
				...
				switch ($row[2]) {
					...
					case 17:
						...
						$intArray1 = PST$b64ToBytes($stringUtil_getFlatValue($value));
						if ($intArray1 == null) {
							$errorId = 4;
...
let $stringFlatten = function($s) {
	let $ucharsBuilder = PST$createNewArray($s[0], 0);
	let $ucharOffset = 0;
	let $strBuilder = [];
	let $q = [];
	$q.push($s);
...
let $stringUtil_split = function($g, $str, $sep) {
	...
	let $o = PST$createNewArray($sz, null);
	let $i = 0;
	while ($i < $sz) {
...
let $stringUtil_trim = function($str, $front, $back) {
	...
	let $newChars = PST$createNewArray($length, 0);
	let $i = 0;
	while ($i < $length) {
...
let $valueArrayIncreaseCapacity = function($oldArr) {
	...
	let $newArr = PST$createNewArray($newCapacity, null);
	let $i = 0;
	while ($i < $oldCapacity) {
		$newArr[$i] = $oldArr[$i];
		$i += 1;
...
let $valueToHumanString = function($value) {
	switch ($value[0]) {
		...
		case 3:
			return $value[1] + '';
		case 4:
			let $floatStr = PST$floatToStr($value[1]);
...
let $xml_performEntitySwaps = function($chars) {
	...
	while ($i < $chars.length) {
		if ($chars[$i] == 37) {
			let $j = 0;
			while ($j < 10) {
				if ($i + $j < $chars.length && $chars[$i + $j] == 58) {
					let $end = $i + $j;
...
let $xml_popQuotedValue = function($ctx) {
	...
	let $chars = [];
	while ($ctx[4] < $ctx[6] && $xml_peekChar($ctx) != 33) {
...
let $xml_popTextValue = function($ctx) {
	let $indexStart = $ctx[4];
	while ($ctx[4] < $ctx[6] && $ctx[5][$ctx[4]] != 59) {
		$xml_popChar($ctx);
	}
	return 0;
};
