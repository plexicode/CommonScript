...
let $_Errors_ThrowImpl = function($type, $t, $s1, $s2) {
	...
	return PST$extCallbacks["throwParserException"]($args);
};
...
let $allocateStringAndTokenIds = function($bundle) {
	...
	let $tokensById = PST$createNewArray($fpByIndex.length, null);
	$i = 0;
	while ($i < $allByteCode.length) {
		let $row = $allByteCode[$i];
		if ($row[1] != null) {
...
let $bsbFrom4Bytes = function($a, $b, $c, $d) {
	let $arr = PST$createNewArray(4, 0);
	$arr[0] = $a;
	$arr[1] = $b;
	$arr[2] = $c;
	$arr[3] = $d;
...
let $bsbFromInt = function($value) {
	...
	if ($value >= 0 && $value < 128) {
		...
		if ($value < 256) {
			$firstByte |= 1;
			$buf = PST$createNewArray(2, 0);
			$buf[0] = $firstByte;
			$buf[1] = $value & 255;
		} else if ($value <= 65535) {
			$firstByte |= 2;
			$buf = PST$createNewArray(3, 0);
			$buf[0] = $firstByte;
			$buf[1] = $value >> 8 & 255;
			$buf[2] = $value & 255;
		} else if ($value <= 16777215) {
			$firstByte |= 3;
			$buf = PST$createNewArray(4, 0);
			$buf[0] = $firstByte;
			$buf[1] = $value >> 16 & 255;
			$buf[2] = $value >> 8 & 255;
			$buf[3] = $value & 255;
...
let $BundleEnumInfo_createFromEntity = function($e) {
	let $sz = $e[1].length;
	let $names = PST$createNewArray($sz, null);
	let $values = PST$createNewArray($sz, 0);
	let $i = 0;
	while ($i < $sz) {
		$names[$i] = $e[0][$i][0];
		$values[$i] = $e[1][$i][8];
...
let $ClassSorter_SortClassesInDeterministicDependencyOrder = function($unorderedClasses, $considerBaseClasses) {
	...
	let $output = PST$createNewArray($keys.length, null);
	$i = 0;
	while ($i < $keys.length) {
...
let $create0 = function($opCode, $token, $stringArg) {
	return $ByteCodeBuffer_fromRow($ByteCodeRow_new($opCode, $token, $stringArg, PST$createNewArray(0, 0)));
};

let $create1 = function($opCode, $token, $stringArg, $arg1) {
	let $args = PST$createNewArray(1, 0);
	$args[0] = $arg1;
...
let $create2 = function($opCode, $token, $stringArg, $arg1, $arg2) {
	let $args = PST$createNewArray(2, 0);
	$args[0] = $arg1;
	$args[1] = $arg2;
	return $ByteCodeBuffer_fromRow($ByteCodeRow_new($opCode, $token, $stringArg, $args));
};
...
let $create3 = function($opCode, $token, $stringArg, $arg1, $arg2, $arg3) {
	let $args = PST$createNewArray(3, 0);
	$args[0] = $arg1;
	$args[1] = $arg2;
	$args[2] = $arg3;
	return $ByteCodeBuffer_fromRow($ByteCodeRow_new($opCode, $token, $stringArg, $args));
...
let $finalizeBreakContinue = function($originalBuffer, $additionalBreakOffset, $allowContinue, $additionalContinueOffset) {
	...
	while ($i < $rows.length) {
		...
		if ($op == -1 || $op == -2) {
			...
			$rows[$i][3] = PST$createNewArray(1, 0);
...
let $flatten = function($buffer) {
	if ($buffer == null) {
		return PST$createNewArray(0, null);
	}
	let $q = [];
	$q.push($buffer);
	let $output = [];
...
let $ParseIfStatement = function($tokens) {
	...
	if ($Tokens_popIfPresent($tokens, "else")) {
		...
		$elseCode = PST$createNewArray(0, null);
	}
...
let $ParseLambda = function($tokens) {
	...
	if ($Tokens_isNext($tokens, "{")) {
		...
		$code = PST$createNewArray(1, null);
...
let $ParseTry = function($tokens) {
	...
	if ($Tokens_isNext($tokens, "finally")) {
		...
		$finallyCode = PST$createNewArray(0, null);
	}
...
let $serializeTryStatement = function($staticCtx, $tryStmnt) {
	...
	let $tryCatchInfo = PST$createNewArray(4, 0);
	$tryCatchInfo[0] = 0;
...
let $StatementResolver_FirstPass_Try = function($resolver, $tryStatement) {
	...
	while ($i < $tryStatement[15].length) {
		...
		if ($cc[0].length > 0) {
			...
			$cc[4] = true;
			$cc[1] = PST$createNewArray(0, null);
		}
...
let $StringArraySlice = function($arr, $skipStart, $skipEnd) {
	...
	let $output = PST$createNewArray($dstLen, null);
	let $i = 0;
	while ($i < $dstLen) {
...
let $StringToUnicodeArray = function($strVal) {
	...
	let $output = PST$createNewArray($bytes.length, 0);
	...
	if ($j < $length) {
		let $trimmedOutput = PST$createNewArray($j, 0);
		while ($j > 0) {
			$j -= 1;
...
let $Tokenize = function($file, $content, $ctx) {
	...
	let $lines = PST$createNewArray($length, 0);
	let $cols = PST$createNewArray($length, 0);
	let $i = 0;
	let $j = 0;
	let $k = 0;
	let $line = 1;
...
let $TokenizerStaticContext_new = function() {
	...
	while ($i < $mcharTokens.length) {
		...
		let $uchars = PST$createNewArray($mcharTok.length, 0);
		let $j = 0;
